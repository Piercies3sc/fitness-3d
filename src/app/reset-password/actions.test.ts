...
const { mockCreateClient, mockCookies, mockGetUser, mockUpdateUser } = vi.hoisted(() => ({
...
describe('updatePassword', () => {
  ...
  it('updates the password only for a valid recovery session and clears its marker', async () => {
    await expect(updatePassword('new-password', 'new-password')).resolves.toEqual({ success: true })
